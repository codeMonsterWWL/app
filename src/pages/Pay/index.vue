...
export default {
  ...
  methods: {
    ...
    async open() {
      //生成二维码地址
      //利用QRCode，把服务器返回的二维码的字符串转化为真正的二维码的地址
      ...
      this.$alert(`<img src=${url} />`, "请微信扫码支付", {
        ...
        confirmButtonText: "已支付完成", //确定按钮的文本
        showClose: false, //弹框右上角的关闭按钮×
        //关闭弹出窗的配置项
        beforeClose: (type, instance, done) => {
          //type:区分点击的是【取消】还是【确定】按钮
          //instance：当前的组件实例
          //done：关闭弹出框的方法
          if (type == "cancel") {
            alert("请联系管理员王文龙");
      ...
      //需要知道支付成功|失败
      //支付成功：路由跳转   支付失败：提示信息
      //当二维码弹出来之后，得一直向服务器问，是否支付成功，不能只发一次，因为不可能瞬间支付，得符合常理
      //如果定时器没有，就开启一个新的定时器
      if (!this.timer) {
        this.timer = setInterval(async () => {
          ...
          if (result.code == 200) {
            ...
            clearInterval(this.timer);
            this.timer = null;
            //保存支付成功返回的code（200）,未支付时，不能点击【已支付成功】
            this.code = 200;
            //关闭弹出框
            this.$msgbox.close();
            //跳转到下一路由：支付成功
