        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 假如购物车，在路由跳转之前，将购买的商品的信息通过请求的形式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  methods: {
    //产品属性值切换高亮,将点击的属性值和他的父亲元素传过去（传父级元素是为了能遍历所点击元素的兄弟元素 ）
    changeActive(saleAttrValue, arr) {
      //排他思想:排他思想：如果有同一组元素，想要某一个元素实现某种样式，需要用到循环的排他思想算法
      // 所有元素全部清除样式
      // 给当前元素设置样式
      // 注意顺序不能颠倒
      //将全部的售卖属性值的isChecked设置为0，
      arr.forEach((item) => {
        item.isChecked = 0;
      });
      //将点击的售卖属性值的isChecked设置为1
      saleAttrValue.isChecked = 1;
    },
    //表单元素修改商品的个数，（纠正用户输入非法字符）
    changeskuNum(event) {
      let value = event.target.value * 1;
      //非法数字：出现NaN或者小于1的，如果输入的是数字，*1之后结果不影响，但是输入的含有汉字，*1后就成NaN了
    ...
    async addShopcar() {
      ...
      //这行代码说白了就是调用仓库中的addOrUpdateShopCart这个函数，而这个函数前面加了async，那么返回的就是一个promise对象，要么成功要么失败 
      //这里再等待addOrUpdateShopCart这个函数返回的promise的成功
      try {
       await this.$store.dispatch("addOrUpdateShopCart", {skuId: this.$route.params.skuid,skuNum: this.skuNum,});
          //如果成功，路由跳转  ，将商品信息带给addcartsuccess组件，addcartsuccess进行展示商品信息，个数，名字，图片，详情信息
          //向下面这么传就行，addcartsuccess组件就能拿到需要的数据，但是就是路径有点丑。
          // this.$router.push({path:'/addcartsuccess',query:{skuInfo:this.skuInfo,skuNum:this.skuNum}})
          //这里我们尝试另一种方法：会话存储（非持久化的，vue是单页面应用，只要未关闭这个页面，就一直存在，但是没必要用本地存储，这样关闭浏览器，再打开浏览器，数据仍然在，浪费资源，没必要）（本地存储和会话存储都不能存对象，因为是k-v的形式，v存储的是字符串），简单的数据通过query带过去，复杂的数据（名字，图片，详情信息）这些用会话存储存起来，addcartsuccess再从会话存储里面读取数据
          sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo))//将对象转化为字符串
